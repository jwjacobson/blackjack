...
class Player:
    ...
    def deal(self):  # Deal initial hands and report them to user
        ...
        for i in range(2):
            self.hand.append(
                deck.pop()
            )  # Since deck is a set, pop is sufficiently random
    ...
    def get_points(
        self,
    ):  # Analyze points in hands, including possibility of immediate win
        global tens
        tens = {
            "a",
            "b",
            "c",
            "d",
        }  # Set "tens" covers 10, J, Q, K, all of which also cause trouble being evaluated as ints
        for card in self.hand:
            if (
                int(card[0], 16) < 10
            ):  # Integers in number cards under 10 correspond to their value
    ...
    def hit(self):  # Allow players to hit until they win, bust, or stand
        if self.is_dealer:
            while self.points <= 16:  # Dealer will hit until 16 points or bust
                ...
                if (
                    self.value_check()
                ):  # value_check returns true if at or over 21; return prevents continuing to "stand"
                    return True
            print(f"\n{self} stands.\n")
        else:
            answers = "yn"
    ...
    def reveal(self, player):  # Only runs if both players stand (haven't won or busted)
        ...
        print(
            f"{self} had {evaluate(hidden_card)}."
        )  # Reveal dealer's hidden card and total points
...
def shuffle():  # Create deck
    ...
    for i in range(13):
        deck.add(
            hex(i + 2)[2:] + "S"
        )  # Cards stored as 2-character string, converted to human readable by evaluate() (below)
...
def main():
    ...
    while game:  # Gameplay loop
        ...
        dealer.deal()  #
        if (
            not player.get_points()
        ):  # Only continue to next gameplay phase if no one has won/lost yet
            if (
                dealer.get_points() != True
            ):  # Specify != True because dealer.get_points() returns showing_points int
                if not player.hit():
                    if not dealer.hit():
